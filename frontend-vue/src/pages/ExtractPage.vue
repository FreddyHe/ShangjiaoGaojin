<template>
  <div class="grid gap-6 max-w-2xl mx-auto">
    ...
    <div v-if="outline" class="card p-6">
      <div class="font-bold text-brand-600 mb-4">类型：{{ outline.type_name }}</div>
...
export default defineComponent({
  components: {
    SectionView: defineComponent({
      ...
      template: `
        <div class="border rounded-xl p-4 bg-surface-50" :class="depth > 0 ? 'ml-6' : ''">
          <div class="font-semibold text-text-primary">{{ section.title }}</div>
          <ul v-if="(section.bullets||[]).length" class="list-disc ml-6 mt-2 text-text-secondary">
